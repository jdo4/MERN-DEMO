...
module.exports = exports = {
  ...
  update: async (req, res) => {
    ...
    if (await DB.USER.findOne({ email: req.body.email, _id : {$ne : req.user?._id} }))
    ...
    if(req.body?.password){
      req.body.password = await helper.hashPassword({ password: req.body?.password });
    }

    let data = await DB.USER.findByIdAndUpdate(req.user?._id, req.body, {
      new: true,
    });
